# LicencePress Architecture Instructions

LicencePress is a modular WordPress platform. Internal plugins and separately installed WordPress plugins are the primary way to add features and integrations. Preserve this architecture in every change.
...
- `src/languages/` contains LicencePress core translation catalogs and compiled language files.
...
- Keep feature-specific behavior out of LicencePress core. Core must not contain conditions, selectors, settings keys, labels, capabilities, or business rules for a particular plugin.
...
- Internal plugins live under `src/Includes/Plugins/<LicencePress>/` and are initialized by `Plugins.php` through the contracts in `PluginsInterface.php`.
- Separately installed plugins register through `licencepress_register_plugin` and follow the same ownership rules. They must not require core changes that reference their slug or class.
- Do not bypass the plugin loader with direct feature requires from `LicencePress.php`, `Plugin.php`, or core bootstrap code.
...
All LicencePress and internal-plugin assets must flow through `src/Assets/Assets.php`.

- Plugin asset classes may define asset lists and page conditions, but must register them through the central `LicencePress\Assets\Assets` service.
...
- LicencePress and its plugins use the repository language scripts to manage translation catalogs and compiled language files.
