...
A separately installed WordPress plugin can extend LicencePress without being placed in the internal extension directory. The bridge is the `licencepress_register_plugin` action.
...
<?php
/**
 * Plugin Name: My LicencePress Extension
 */

use LicencePress\Includes\Plugins\Plugins;
use LicencePress\Includes\Plugins\PluginInterface;

add_action('licencepress_register_plugin', static function (Plugins $plugins): void {
...
final class MyPluginExtension implements PluginInterface {
    ...
    public function get_description(): string { return 'Adds a LicencePress integration.'; }
...
Use `PermissionHelper` for protected routes and import `Response` from `LicencePress\API` when using the host response envelope. Shortcode callbacks must return rendered output.
...
Implement settings providers for defaults and generated settings pages. Read through `LicencePress\Includes\Settings\Settings` and sanitize all submitted values before storing them. External extensions own their translation catalogs and language files.
...
1. Declare LicencePress as a dependency or check for its classes.
2. Load the extension class before the registration hook runs.
3. Register on `licencepress_register_plugin`.
...
7. Test activation with LicencePress active and inactive.
