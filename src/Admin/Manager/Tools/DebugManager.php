...
final class DebugManager extends Manager {
    ...
    public function render_page_content(): void {
        ...
        <?php if ( is_array( $diagnostics ) ) : ?>
            ...
            <div class="row g-4 mb-4">
                <?php foreach ( [ 'curl' => __( 'cURL diagnostic', 'licencepress' ), 'http_graph' => __( 'HTTP Graph diagnostic', 'licencepress' ) ] as $transport => $title ) : ?>
        ...
        <div class="card shadow-sm mb-4">
            <div class="card-body">
                ...
                <form method="post" action="<?php echo esc_url( admin_url( 'admin.php?page=licencepress-tools&tool=debug' ) ); ?>">
                    ...
                    <?php echo FormFieldHelper::input( 'licencepress_run_graph_diagnostics', '1', [ 'type' => 'hidden' ] ); ?>
                    <?php echo FormFieldHelper::button( __( 'Run Graph diagnostics', 'licencepress' ), [ 'type' => 'submit', 'class' => 'btn-primary' ] ); ?>
                </form>
    ...
    public function render( array $values ): void {
        ...
        echo '<tr><th scope="row">' . wp_kses_post( FormFieldHelper::label( $field_id, __( 'Debug logging', 'licencepress' ), $field ) ) . '</th><td>' . wp_kses_post( FormFieldHelper::checkbox( 'licencepress_tools[debug_logging]', '1', __( 'Enable LicencePress debug logging', 'licencepress' ), [ 'id' => $field_id, 'checked' => ! empty( $values['debug_logging'] ) ] ) ) . '</td></tr>';
        ...
        echo '<tr><th scope="row">' . wp_kses_post( FormFieldHelper::label( $field_id, __( 'Console logging', 'licencepress' ), $field ) ) . '</th><td>' . wp_kses_post( FormFieldHelper::checkbox( 'licencepress_tools[console_logging]', '1', __( 'Enable browser console logging', 'licencepress' ), [ 'id' => $field_id, 'checked' => ! empty( $values['console_logging'] ) ] ) ) . '</td></tr>';
    }
}