...
final class ResetManager extends Manager {
	...
	public function __construct( ?LoaderHelper $loader = null ) {
		( $loader ?? new LoaderHelper() )->register_component(
			...
			array(
				array(
					'type'     => 'action',
					'hook'     => 'admin_post_licencepress_reset',
		...
		<div class="card shadow-sm">
			<div class="card-body">
				...
				<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
					<?php echo FormFieldHelper::input( 'action', 'licencepress_reset', array( 'type' => 'hidden' ) ); ?>
					...
					<?php echo FormFieldHelper::select( 'scope', $this->scope_options(), 'core', array( 'id' => 'licencepress-reset-scope' ) ); ?>
					<fieldset class="mt-4" id="licencepress-reset-plugins" data-licencepress-reset-plugins hidden>
						...
						foreach ( $this->plugin_options() as $slug => $plugin ) {
							echo FormFieldHelper::checkbox( 'plugins[]', $slug, $plugin['name'], array( 'id' => 'licencepress-reset-' . $slug ) );
						}
						?>
					</fieldset>
					<div class="mt-4">
						...
						echo FormFieldHelper::checkbox(
							'confirm',
							'1',
							__( 'I understand that this action cannot be undone.', 'licencepress' ),
							array(
								'id'       => 'licencepress-reset-confirm',
								'required' => true,
							)
						);
						?>
					...
					<div class="mt-4">
						...
						echo FormFieldHelper::button(
							__( 'Reset selected data', 'licencepress' ),
							array(
								'type'  => 'submit',
								'class' => 'btn-danger',
							)
	...
	public function handle_reset(): void {
		if ( ! AjaxHelper::is_method( 'POST' ) || ! PermissionHelper::can( 'licencepress_tools_reset' ) || ! AjaxHelper::has_valid_nonce( 'licencepress_reset', 'licencepress_reset_nonce' ) ) {
			wp_die( esc_html__( 'The reset request could not be authorized.', 'licencepress' ), '', array( 'response' => 403 ) );
		}
	...
	private function scope_options(): array {
		return array(
			...
			'core'    => __( 'LicencePress core only', 'licencepress' ),
