document.addEventListener('DOMContentLoaded', () => {
    ...
    const showPluginNotice = (alertMarkup) => {
        const container = root.querySelector('#licencepress-settings-panel .licencepress-settings-tab-content');
        if (!container || !alertMarkup) return;

        container.querySelectorAll('[data-licencepress-alert]').forEach((notice) => notice.remove());
    ...
    const togglePlugin = (toggle) => {
        ...
        const body = new URLSearchParams({
            action: 'licencepress_toggle_plugin',
    ...
    const savePluginSettings = (button) => {
        const modal = button.closest('.licencepress-plugin-settings-modal');
        ...
        body.set('action', 'licencepress_save_plugin_settings');
    ...
    root.addEventListener('click', (event) => {
        ...
        // Close modal
        const dismiss = event.target.closest?.('.licencepress-plugin-settings-modal [data-bs-dismiss="modal"]');
        if (dismiss) {
            event.preventDefault();
            const modal = dismiss.closest('.licencepress-plugin-settings-modal');
    ...
    root.addEventListener('change', (event) => {
        const toggle = event.target.closest?.('[data-licencepress-plugin-toggle]');
        if (toggle) {
            togglePlugin(toggle);
        }
    }, true);
