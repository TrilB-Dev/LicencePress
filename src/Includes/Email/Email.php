...
final class Email {
    ...
    public static function send( $to, string $subject, string $message, array $headers = [], array $attachments = [] ): bool {
        ...
        $recipients = is_array( $to ) ? array_filter( array_map( 'trim', $to ) ) : [ trim( (string) $to ) ];
        if ( empty( $recipients ) ) {
            return false;
        }
     ...
     * @return object|null
     */
    public static function template( string $type, array $context = [] ) {
     ...
     * @return bool
     */
    public static function send_template( $to, string $type, array $context = [], array $headers = [], array $attachments = [] ): bool {
        ...
        $subject = method_exists( $template, 'subject' ) ? (string) $template->subject() : 'LPEmail-Admin';
    ...
    private static function resolve_template_class( string $type ): string {
        $type = trim( $type, '\/' );
        $segments = array_values( array_filter( preg_split( '/[\\\/]+/', $type ) ?: [], 'strlen' ) );
