<?php
/**
 * LicencePress encryption helpers.
 *
...
final class EncryptionHelper {
	/**
	 * Add the LicencePress encryption key to wp-config.php when it is not defined.
	 *
	...
	public static function ensure_configured(): bool {
		...
		if ( null === $config_path || ! is_readable( $config_path ) || ! is_writable( $config_path ) ) {
			...
			error_log( 'LicencePress: wp-config.php was not found or is not writable; encryption key was not created.' );
			return false;
		}
		...
		if ( false === $config_content ) {
			// phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log -- This is only used when the config file cannot be updated safely.
			error_log( 'LicencePress: could not read wp-config.php; encryption key was not created.' );
			return false;
		}
		...
		try {
			...
			error_log( 'LicencePress: failed to generate the encryption key: ' . $exception->getMessage() );
			return false;
		}
		...
		if ( false !== $marker_position ) {
			...
			if ( false === $settings_position ) {
				// phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log -- This is only used when the config file cannot be updated safely.
				error_log( 'LicencePress: could not find a safe insertion point in wp-config.php.' );
				return false;
			}
		...
		if ( false === $handle || ! flock( $handle, LOCK_EX ) ) {
			...
			// phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log -- This is only used when the config file cannot be updated safely.
			error_log( 'LicencePress: could not lock wp-config.php; encryption key was not created.' );
			return false;
		}
		...
		if ( ! $success ) {
			// phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log -- This is only used when the config file cannot be updated safely.
			error_log( 'LicencePress: could not write the encryption key to wp-config.php.' );
			return false;
		}
