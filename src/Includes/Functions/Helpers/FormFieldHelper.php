...
final class FormFieldHelper {
    ...
    public static function dropdown_button( string $label, array $items = [], array $options = [] ): string {
        ...
        if ( ! empty( $options['split'] ) ) {
            $toggle .= '<span class="visually-hidden">' . esc_html__( 'Toggle Dropdown', 'licencepress' ) . '</span>';
        }
    ...
    public static function button_toolbar( array $groups = [], array $options = [] ): string {
        ...
        $attributes['id'] = (string) ( $options['id'] ?? sanitize_title( 'licencepress-button-toolbar' ) );
    ...
    private static function render_bootstrap_select( string $name, array $options ): string {
        ...
        $settings = [
            ...
            'live_search_placeholder' => [ 'live-search-placeholder', __( 'Search or create', 'licencepress' ) ],
            'open_options_text' => [ 'open-options-text', __( 'Create "{0}"', 'licencepress' ) ],
            'selected_text_format' => [ 'selected-text-format', 'count' ],
            'selected_items_style' => [ 'selected-items-style', 'tags' ],
            'selected_tag_remove_label' => [ 'selected-tag-remove-label', __( 'Remove', 'licencepress' ) ],
            'placeholder' => [ 'placeholder', __( 'Select options', 'licencepress' ) ],
            'width' => [ 'width', '100%' ],
            'size' => [ 'size', null ],
