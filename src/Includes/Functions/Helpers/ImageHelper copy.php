    ...
    * @param string $type The asset type: core or an internal plugin slug.
    ...
    public static function get_image_url( string $type, string $file ): string {
        ...
        if ( 'core' === strtolower( trim( $type ) ) ) {
            return LICENCEPRESS_ASSETS_URL . '/images/' . $file;
        }
    ...
    private static function get_plugin_directory( string $plugin_slug ): string {
        ...
        if ( '' === $plugin_slug || ! defined( 'LICENCEPRESS_PLUGINS' ) || ! is_dir( LICENCEPRESS_PLUGINS ) ) {
            return '';
        }

        $plugin_slug_variants = [ $plugin_slug ];
        if ( str_starts_with( $plugin_slug, 'licencepress-' ) ) {
        ...
        foreach ( scandir( LICENCEPRESS_PLUGINS ) ?: [] as $directory ) {
            if ( '.' !== $directory && '..' !== $directory && is_dir( LICENCEPRESS_PLUGINS . '/' . $directory ) && in_array( strtolower( $directory ), $plugin_slug_variants, true ) ) {
                return $directory;
            }
        }
