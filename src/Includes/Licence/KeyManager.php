...
final class KeyManager {
	...
	public static function ensure_configured(): bool {
		...
		// phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations -- wp-config.php is the active bootstrap configuration and must be checked before write.
		if ( null === $config_path || ! is_readable( $config_path ) || ! is_writable( $config_path ) ) {
			return false;
		}

		// phpcs:ignore WordPress.WP.AlternativeFunctions.file_get_contents_file_get_contents -- wp-config.php is the active bootstrap configuration and must be read before insertion.
		...
		// phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations -- wp-config.php is the active bootstrap configuration and must be updated atomically.
