...
final class LicenceGenerator {
	 ...
	 * @throws \RuntimeException If the payload cannot be encoded or encrypted.
	 */
	public static function generate(
		...
		$normalized_site_url = null;
		$site_hash           = null;
		if ( null !== $site_url && '' !== $site_url ) {
			$normalized_site_url = self::normalize_domain( $site_url );
			$site_hash           = hash( 'sha256', $normalized_site_url );
		}
	...
	private static function normalize_domain( string $value ): string {
		...
		$normalized = preg_replace( '/^www\./', '', $value );
