...
namespace LicencePress\Includes\Plugins\FontAwesome\Includes;
...
class IconPicker {
    ...
    private function init() {
        ...
        if ( function_exists( 'add_action' ) ) {
            $this->loader->register_component( $this, [
                [ 'type' => 'action', 'hook' => 'wp_ajax_licencepress_fontawesome_search_icons', 'callback' => 'ajax_search_icons' ],
                [ 'type' => 'action', 'hook' => 'wp_ajax_licencepress_fontawesome_get_icon_data', 'callback' => 'ajax_get_icon_data' ],
            ] )->run();
        }
    }
    ...
    public function ajax_search_icons() {
        if ( ! AjaxHelper::has_valid_nonce( 'licencepress_fontawesome_picker' ) ) {
    ...
    public function search_icons( $search = '', $pack = 'classic', $style = 'solid', $page = 1, $per_page = 50 ) {
        ...
        try {
            ...
            LoggerHelper::write_log( 'LicencePress FontAwesome IconPicker: Error searching icons: ' . $e->getMessage() );
    ...
    public function get_icon_data( $icon_name, $pack = 'classic', $style = 'solid' ) {
        ...
        try {
            ...
            LoggerHelper::write_log( 'LicencePress FontAwesome IconPicker: Error getting icon data: ' . $e->getMessage() );
            return null;
        }
    }
    ...
    private function get_available_icons_from_api( $pack, $style ) {
        ...
        try {
            ...
            LoggerHelper::write_log( 'LicencePress FontAwesome IconPicker: Error getting icons from API: ' . $e->getMessage() );
            return array();
        }
    }
    ...
    private function query_fontawesome_api( $pack, $style ) {
        ...
        try {
            ...
            LoggerHelper::write_log( 'LicencePress FontAwesome IconPicker: Executing GraphQL query for pack=' . $pack . ', style=' . $style );
            ...
            if ( json_last_error() !== JSON_ERROR_NONE ) {
                LoggerHelper::write_log( 'LicencePress FontAwesome IconPicker: JSON decode error: ' . json_last_error_msg() );
                return array();
            }

            if ( ! isset( $response['data']['release']['icons'] ) ) {
                LoggerHelper::write_log( [
                    'message' => 'LicencePress FontAwesome IconPicker: Invalid API response structure.',
                    'response' => $response,
                ] );
                return array();
            }
            ...
            LoggerHelper::write_log( 'LicencePress FontAwesome IconPicker: Retrieved ' . count( $all_icons ) . ' total icons from API' );
            ...
            LoggerHelper::write_log( 'LicencePress FontAwesome IconPicker: Filtered to ' . count( $icons ) . ' icons for pack=' . $pack . ', style=' . $style );
            return $icons;

        } catch ( Exception $e ) {
            LoggerHelper::write_log( 'LicencePress FontAwesome IconPicker: Error querying FontAwesome API: ' . $e->getMessage() );
            LoggerHelper::write_log( 'LicencePress FontAwesome IconPicker: Exception trace: ' . $e->getTraceAsString() );
            return array();
        }
    }
    ...
    public function render_picker( $input_name, $selected_icon = '', $args = array() ) {
        $args = wp_parse_args( $args, array(
            'id' => uniqid( 'fa-picker-' ),
            'input_id' => '',
            'class' => 'licencepress-fa-picker',
            'button_text' => __( 'Choose Icon', 'licencepress' ),
            'preview_size' => '24px',
        ) );
        ...
        ob_start();
        ?>
        <div class="licencepress-fa-picker-container" id="<?php echo esc_attr( $args['id'] ); ?>">
                 <?php echo wp_kses_post( FormFieldHelper::input( $input_name, $selected_icon, [ 'type' => 'hidden', 'id' => $args['input_id'], 'class' => 'licencepress-fa-picker-input' ] ) ); ?>

                <button type="button"
                    class="btn btn-secondary licencepress-fa-picker-button"
                    data-picker-id="<?php echo esc_attr( $args['id'] ); ?>">
                <span class="licencepress-fa-picker-preview">
                    <?php if ( $selected_data ) : ?>
                        <i class="<?php echo esc_attr( $selected_data['class'] ); ?>"
                           style="font-size: <?php echo esc_attr( $args['preview_size'] ); ?>"></i>
                    <?php endif; ?>
                </span>
                <span class="licencepress-fa-picker-text">
                    <?php echo esc_html( $args['button_text'] ); ?>
                </span>
            </button>

            <div class="licencepress-fa-picker-modal" style="display: none;">
                <div class="licencepress-fa-picker-overlay"></div>
                <div class="licencepress-fa-picker-dialog">
                    <div class="licencepress-fa-picker-header">
                        <h3><?php _e( 'Choose an Icon', 'licencepress' ); ?></h3>
                        <button type="button" class="licencepress-fa-picker-close">&times;</button>
                    </div>

                    <div class="licencepress-fa-picker-search">
                        <?php echo wp_kses_post( FormFieldHelper::input( 'licencepress_fa_search', '', [ 'type' => 'search', 'class' => 'licencepress-fa-picker-search-input', 'placeholder' => __( 'Search icons...', 'licencepress' ) ] ) ); ?>
                        <?php echo wp_kses_post( FormFieldHelper::select( 'licencepress_fa_pack', [ 'classic' => __( 'Classic', 'licencepress' ), 'duotone' => __( 'Duotone', 'licencepress' ), 'sharp' => __( 'Sharp', 'licencepress' ), 'sharp-duotone' => __( 'Sharp Duotone', 'licencepress' ), 'brands' => __( 'Brands', 'licencepress' ) ], 'classic', [ 'class' => 'licencepress-fa-picker-pack-filter' ] ) ); ?>
                        <?php echo wp_kses_post( FormFieldHelper::select( 'licencepress_fa_style', [ 'solid' => __( 'Solid', 'licencepress' ), 'regular' => __( 'Regular', 'licencepress' ), 'light' => __( 'Light', 'licencepress' ), 'thin' => __( 'Thin', 'licencepress' ) ], 'solid', [ 'class' => 'licencepress-fa-picker-style-filter' ] ) ); ?>
                    </div>

                    <div class="licencepress-fa-picker-results">
                        <div class="licencepress-fa-picker-loading">
                            <?php _e( 'Loading icons...', 'licencepress' ); ?>
                        </div>
                    </div>

                    <div class="licencepress-fa-picker-footer">
                        <button type="button" class="btn btn-secondary licencepress-fa-picker-cancel">
                            <?php _e( 'Cancel', 'licencepress' ); ?>
                        </button>
                        <button type="button" class="btn btn-primary licencepress-fa-picker-select" disabled>
                            <?php _e( 'Select Icon', 'licencepress' ); ?>
                        </button>
                    </div>
                </div>
            </div>
