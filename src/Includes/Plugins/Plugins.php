<?php
/**
 * LicencePress - Plugins
 *
 * Handles discovery and loading of LicencePress plugin modules.
 *
 * @package LicencePress
 * @subpackage Includes\LicencePress\Plugins
 * @since 1.0.0
 */
 ...
 * Class Plugins
 *
 * Manages the discovery, loading, and initialization of LicencePress plugin modules.
 */
class Plugins {
	/**
	...
	public function init(): void {
		...
		/**
		 * Allow WordPress plugins to register LicencePress extensions.
		 *
		 * Plugins installed via the normal WordPress plugin system can hook
		 * into this action and call LicencePress\Plugins::register_plugin().
		 */
		do_action( 'licencepress_register_plugin', $this );
	}
	/**
	...
	private function resolve_plugin_directory(): string {
		$path     = trim( Settings::get( 'licencepress_plugin_directory', LICENCEPRESS_PLUGINS ) );
		$resolved = $this->is_absolute_path( $path )
			? untrailingslashit( $path )
			: untrailingslashit( LICENCEPRESS_ROOT ) . '/' . ltrim( str_replace( '\\', '/', $path ), '/' );

		if ( ! is_dir( $resolved ) ) {
			return LICENCEPRESS_PLUGINS;
		}
	...
	private function load_plugin_file( string $file ): void {
		...
		try {
			...
			if ( ! class_exists( $fqcn ) || ! is_a( $fqcn, PluginInterface::class, true ) ) {
				LoggerHelper::write_log( sprintf( 'LicencePress plugin file %s does not declare a PluginInterface implementation.', $file ) );
				return;
			}
			...
			if ( ! $instance instanceof PluginInterface ) {
				LoggerHelper::write_log( sprintf( 'LicencePress plugin %s does not implement PluginInterface.', $fqcn ) );
				return;
			}
			...
			LoggerHelper::write_log( sprintf( 'LicencePress plugin loader failed to require file %s: %s', $file, $e->getMessage() ) );
		}
	}
	/**
	...
	private function initialize_plugin( PluginInterface $plugin ): void {
		...
		try {
			...
			LoggerHelper::write_log( sprintf( 'LicencePress plugin %s failed to initialize: %s', $plugin->get_slug(), $e->getMessage() ) );
		}
	}
	/**
